  ...
  return (
    <div className={`mt-4 ${disabled && 'pointer-events-none opacity-50'} ${fullWidth && 'w-full'}`}>
      <div className='relative inline-flex flex-col m-0 p-0 align-top w-full'>
        <label
          className={cn(`absolute left-0 top-0 p-1 block truncate z-1 text-[12px] font-medium translate-x-[8px] translate-y-[-14px] bg-surface max-w-[calc(133% - 32px)] text-primary ${hasError && 'text-error'} ${disabled && 'text-onSurface'}`,)}>{label}{required && ' *'}</label>
